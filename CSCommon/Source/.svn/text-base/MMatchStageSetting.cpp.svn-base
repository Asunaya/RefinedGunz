...
void MMatchStageSetting::SetDefault()
{
	...
	strcpy(m_StageSetting.szMapName, MMATCH_DEFAULT_STAGESETTING_MAPNAME);
...
{ 
	strcpy(m_StageSetting.szMapName, pszName); 

	// MapIndex까지 함께 세팅해준다.
	m_StageSetting.nMapIndex = 0;
	for (int i = 0; i < MMATCH_MAP_MAX; i++)
	{
		if (!stricmp(g_MapDesc[i].szMapName, pszName))
		{
...
void MMatchStageSetting::SetMapIndex(int nMapIndex)
{
	...
	{
		strcpy(m_StageSetting.szMapName, g_MapDesc[nMapIndex].szMapName); 
	}
}
