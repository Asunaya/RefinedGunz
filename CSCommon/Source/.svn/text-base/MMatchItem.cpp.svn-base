...
void MMatchItemDescMgr::ParseItem(MXmlElement& element)
{
	...
	for (int i = 0; i < nAttrCount; i++)
	{
		...
		{
			strcpy(pNewDesc->m_szName, MGetStringResManager()->GetStringFromXml(szAttrValue));
		}
		else if (!stricmp(szAttrName, MICTOK_TYPE))
		{
			if (!stricmp(szAttrValue, "melee")) pNewDesc->m_nType = MMIT_MELEE;
			else if (!stricmp(szAttrValue, "range")) pNewDesc->m_nType = MMIT_RANGE;
			else if (!stricmp(szAttrValue, "equip")) pNewDesc->m_nType = MMIT_EQUIPMENT;
			else if (!stricmp(szAttrValue, "custom")) pNewDesc->m_nType = MMIT_CUSTOM;
			else _ASSERT(0);
		}
		else if (!stricmp(szAttrName, MICTOK_RES_SEX))
		{
			if (!stricmp(szAttrValue, "m")) pNewDesc->m_nResSex = 0;
			else if (!stricmp(szAttrValue, "f")) pNewDesc->m_nResSex = 1;
			else if (!stricmp(szAttrValue, "a")) pNewDesc->m_nResSex = -1;
			else _ASSERT(0);
		}
		...
		{
			// 슬롯 타입
			if (!stricmp(szAttrValue, "none"))			pNewDesc->m_nSlot = MMIST_NONE;
			else if (!stricmp(szAttrValue, "melee"))	pNewDesc->m_nSlot = MMIST_MELEE;
			else if (!stricmp(szAttrValue, "range"))	pNewDesc->m_nSlot = MMIST_RANGE;
			else if (!stricmp(szAttrValue, "custom"))	pNewDesc->m_nSlot = MMIST_CUSTOM;
			else if (!stricmp(szAttrValue, "head"))		pNewDesc->m_nSlot = MMIST_HEAD;
			else if (!stricmp(szAttrValue, "chest"))	pNewDesc->m_nSlot = MMIST_CHEST;
			else if (!stricmp(szAttrValue, "hands"))	pNewDesc->m_nSlot = MMIST_HANDS;
			else if (!stricmp(szAttrValue, "legs"))		pNewDesc->m_nSlot = MMIST_LEGS;
			else if (!stricmp(szAttrValue, "feet"))		pNewDesc->m_nSlot = MMIST_FEET;
			else if (!stricmp(szAttrValue, "finger"))	pNewDesc->m_nSlot = MMIST_FINGER;
			else if (!stricmp(szAttrValue, "extra"))	pNewDesc->m_nSlot = MMIST_EXTRA;
			else _ASSERT(0);
		}
		else if (!stricmp(szAttrName, MICTOK_WEAPON))
		{
			...
			else if (!stricmp(szAttrValue, "dagger"))		pNewDesc->m_nWeaponType = MWT_DAGGER;
			else if (!stricmp(szAttrValue, "dualdagger"))	pNewDesc->m_nWeaponType = MWT_DUAL_DAGGER;
			else if (!stricmp(szAttrValue, "katana"))		pNewDesc->m_nWeaponType = MWT_KATANA;
			else if (!stricmp(szAttrValue, "greatsword"))	pNewDesc->m_nWeaponType = MWT_GREAT_SWORD;
			else if (!stricmp(szAttrValue, "doublekatana"))	pNewDesc->m_nWeaponType = MWT_DOUBLE_KATANA;

			else if (!stricmp(szAttrValue, "pistol"))		pNewDesc->m_nWeaponType = MWT_PISTOL;
			else if (!stricmp(szAttrValue, "pistolx2"))		pNewDesc->m_nWeaponType = MWT_PISTOLx2;
			else if (!stricmp(szAttrValue, "revolver"))		pNewDesc->m_nWeaponType = MWT_REVOLVER;
			else if (!stricmp(szAttrValue, "revolverx2"))	pNewDesc->m_nWeaponType = MWT_REVOLVERx2;
			else if (!stricmp(szAttrValue, "smg"))			pNewDesc->m_nWeaponType = MWT_SMG;
			else if (!stricmp(szAttrValue, "smgx2"))		pNewDesc->m_nWeaponType = MWT_SMGx2;
			else if (!stricmp(szAttrValue, "shotgun"))		pNewDesc->m_nWeaponType = MWT_SHOTGUN;
			else if (!stricmp(szAttrValue, "sawedshotgun"))	pNewDesc->m_nWeaponType = MWT_SAWED_SHOTGUN;
			else if (!stricmp(szAttrValue, "rifle"))		pNewDesc->m_nWeaponType = MWT_RIFLE;
			else if (!stricmp(szAttrValue, "machinegun"))	pNewDesc->m_nWeaponType = MWT_MACHINEGUN;
			else if (!stricmp(szAttrValue, "rocket"))		pNewDesc->m_nWeaponType = MWT_ROCKET;
			else if (!stricmp(szAttrValue, "snifer"))		pNewDesc->m_nWeaponType = MWT_SNIFER;

			else if (!stricmp(szAttrValue, "medkit"))		pNewDesc->m_nWeaponType = MWT_MED_KIT;
			else if (!stricmp(szAttrValue, "repairkit"))	pNewDesc->m_nWeaponType = MWT_REPAIR_KIT;
			else if (!stricmp(szAttrValue, "bulletkit"))	pNewDesc->m_nWeaponType = MWT_BULLET_KIT;
			else if (!stricmp(szAttrValue, "flashbang"))	pNewDesc->m_nWeaponType = MWT_FLASH_BANG;
			else if (!stricmp(szAttrValue, "frag"))			pNewDesc->m_nWeaponType = MWT_FRAGMENTATION;
			else if (!stricmp(szAttrValue, "smoke"))		pNewDesc->m_nWeaponType = MWT_SMOKE_GRENADE;
			else if (!stricmp(szAttrValue, "skill"))		pNewDesc->m_nWeaponType = MWT_SKILL;
			else if (!stricmp(szAttrValue, "food"))			pNewDesc->m_nWeaponType = MWT_FOOD;

			// enchant
			else if (!stricmp(szAttrValue, "enchant_fire"))			pNewDesc->m_nWeaponType = MWT_ENCHANT_FIRE;
			else if (!stricmp(szAttrValue, "enchant_cold"))			pNewDesc->m_nWeaponType = MWT_ENCHANT_COLD;
			else if (!stricmp(szAttrValue, "enchant_lightning"))	pNewDesc->m_nWeaponType = MWT_ENCHANT_LIGHTNING;
			else if (!stricmp(szAttrValue, "enchant_poison"))		pNewDesc->m_nWeaponType = MWT_ENCHANT_POISON;
		...
		{
			strcpy(pNewDesc->m_szMeshName, szAttrValue);
		}
		...
		else if( !stricmp(szAttrName, MICTOK_BONUS_DUPLICATE) )
		{
			if( 0 == stricmp("false", szAttrValue) )
