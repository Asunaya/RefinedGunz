...
v3 LagCompManager::GetHeadPosition(const v3 & Origin, MMatchSex Sex, ZC_STATE_LOWER v, int Frame)
{
	...
	if (!Node)
	{
		MGetMatchServer()->LogF(MMatchServer::LOG_ALL, "GetHeadPosition -- Can't find node!");
		return v3(0, 0, 0);
	}

	return Node->GetPosValue(Frame);
}