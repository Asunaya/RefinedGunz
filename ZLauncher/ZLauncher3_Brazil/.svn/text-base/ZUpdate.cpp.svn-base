...
#else
		sprintf( szMsg, "[ZUpdatePatchNode] Check Valid : %s (not exist), UPDATE!", GetFileName());
#endif
...
#else
	sprintf( szMsg, "[ZUpdatePatchNode] Check Valid : %s, %s", GetFileName(), ((m_bValidate) ? "PASS":"UPDATE!"));
#endif
	if ( m_bValidate)
		PutLog( szMsg);
	else
...
bool ZUpdate::Create( const char* pszAddress, unsigned long nPort, const char* pszDefDirectory, const char* pszID, const char* pszPassword)
{
	...
	{
		sprintf( m_szLastError, "The update server is not responding or is not running right now. Please try again after a while. Sorry for your inconvenience.");
	...
	{
		sprintf( m_szLastError, "Please check your internet connection.");
...
bool ZUpdate::CheckValidFromPatchList()
{
	...
	for ( ZUpdatePatchList::iterator itr = m_pUpdatePatchList.begin();  itr != m_pUpdatePatchList.end();  itr++)
	{
		...
		if ( m_bStopUpdate)
		{
			sprintf( m_szLastError, "The update must be completed before it runs.");
		...
		if ( !(*itr)->IsValid())
		{
			...
			for ( int nRetry = 1;  nRetry <= DOWNLOAD_RETRY_COUNT;  nRetry++)
			{
				...
				else if ( nRetry == DOWNLOAD_RETRY_COUNT)
				{
					...
					sprintf( m_szLastError, "Internet connection closed or the file is read-only.");
