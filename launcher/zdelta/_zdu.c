/* file _zdu.c; copyright Dimitre Trendafilov (2003)
 * command line frontend for decompressing zdelta difference for files <= 1MB
...
static const char usage_single[] = "usage: _zdu Reference [Target [Delta]]";
static const char usage_multi[]  = "       _zdu Reference1 Reference2";
...
int main_multi(int argc, char *argv[]){
  ...
  d_size = fread((void *) delta, sizeof(Byte), (size_t) BUF_SIZE, stdin);
  ...
  /* compress the data */
  if(zd_uncompressN(ref_buf,ref_s,ref_n,tar_buf,&tar_s,delta,d_size) == ZD_OK){
    ...
    if(tar_s !=  fwrite(tar_buf,sizeof(char),tar_s ,stdout)) perror("ouput");
  }
...
int main_single(int argc, char *argv[]){
  ...
  /* compress the data */
  if(zd_uncompress(ref_buf, ref_s, tar_buf, &tar_s, delta, d_size) == ZD_OK){ 
