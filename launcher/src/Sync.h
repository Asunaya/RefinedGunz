...
// Creates a .sync file for a file.
// Returns true on success, or false on error.
inline bool MakeSyncFile(const char* OutputFilePath, const char* InputFilePath);
...
struct RemoteFile
{
	...
	// A map, for mapping rolling hashes to indices into the above list.
...
inline bool DownloadSyncFile(RemoteFile& Remote, const char* SyncFileURL,
	DownloadManagerType& DownloadManager)
{
	...
	{
		Log.Debug("Callback invoked -- Buffer = %p, Size = %zu\n",
			Buffer, Size);
	...
	const auto ret = DownloadFile(DownloadManager,
		SyncFileURL, LauncherConfig::PatchPort,
		std::ref(Callback));
...
inline bool CalculateBlocks(RemoteFile& Remote, BlockCounts& Counts, const char* LocalFilePath)
{
	...
	// Contains four blocks, so that the window can move.
	// This is used as a circular buffer, so arithmetic on indices into it should be wrapped with
	// Wrap.
	constexpr auto FileBufferSize = int(BlockSize * 4);
	auto FileBufferMemory = std::make_unique<u8[]>(FileBufferSize);
	auto* FileBuffer = FileBufferMemory.get();
	...
	{
		Log.Debug(4, "Reading %d-%d, LocalFile.tell() = %llu\n",
	...
	{
		// Visual for BlockSize = 4, OldWindowStartIndex = 0, LastLoadEndIndex = 5, AdvanceOffset = 2.
		//
		// [ x | x | - | - | - | - | x | x ]
		//       ^   ^       ^   ^   ^
		//       |   |       |   |   WindowEndIndex
		...
		{
			Log.Debug(4, "AdvanceWindow -- Reading\n"
	...
	auto OnReachedEnd = [&](int Advanced)
	{
		...
		Log.Debug(4, "Last block found at local file offset %llu\n", Remote.LastBlock.LocalFileOffset);
	};
	...
	while (true)
	{
		...
		{
			Log.Debug("Rolling hash matches a block\n");
			...
			for (auto&& Pair : Range)
			{
				...
				Log.Debug(4, "Block value %zu found at local file offset %llu\n",
	...
	if (!LastBlockFound)
		Log.Debug(4, "Last block was not found\n");
...
void MakeNewFilePath(char(&OutputFilePath)[OutputSize], const char* LocalFilePath)
{
	...
	u64* SizeOutput)
{
		...
		u64 OutputOffset) // The offset in the remote/new file.
	{
		...
		else
		{
			...
			Log.Debug(3, "CreateNewFile -- Calling DownloadFile with URL = %s, Port = %d, Range = %s\n",
				RemoteFileURL, LauncherConfig::PatchPort, Range);

			DownloadFile(DownloadManager,
				RemoteFileURL, LauncherConfig::PatchPort,
				Callback, Range);
	...
	BlockCounts* BlockCountsOutput)
{
	...
	auto Success = DownloadSyncFile(Remote, SyncFileURL, DownloadManager);
	if (!Success)
	{
		Log.Error("Failed to download sync file %s\n", SyncFileURL);
		return false;
	}
	...
	Success = CalculateBlocks(Remote, Counts, LocalFilePath);
	if (!Success)
	{
		Log.Error("Failed to calculate blocks for file %s\n", LocalFilePath);
		return false;
	}

	Log.Info("%zu unmatching blocks, %zu matching blocks\n",
		Counts.UnmatchingBlocks, Counts.MatchingBlocks);
	...
	if (!Success)
	{
		Log.Error("Failed to create new file %s from remote %s\n", LocalFilePath, RemoteFileURL);
		return false;
	}
