...
template <> inline CString Cvt(const long& x)    { return strprintf("%l", x); };
template <> inline CString Cvt(const double& x)  { return strprintf("%f", x); }
template <> inline long    Cvt(const CString& x) { return atol(x.c_str()); }
template <> inline double  Cvt(const CString& x) { return atof(x.c_str()); }
...
struct CDBField
{
	...
	auto AsString() const { return Get<CString>(); }

	bool IsNull()   const;

	variant<long, double, CString> var{0l};
	CString Name;
	SQLLEN LenOrInd;
...
struct CODBCRecordset
{
	...
	bool Open(const CString& lpszSQL, u32 nOpenType = 0, u32 dwOptions = 0);
	void Close();
	void MoveNext();
	CDBField& Field(const char* szName);

	CDBBinary SelectBinary(const CString& strQuery);
	bool InsertBinary(const CString& strQuery, const u8* pData, int nSize);
