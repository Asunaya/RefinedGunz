...
/*
	...
	if (pWidget!=NULL)
	{
		...
		{
			char szTemp[128];
			sprintf(szTemp, "%s (HP:%d, AP:%d)", m_pTargetCharacter->GetProperty()->szName,
...
void ZObserver::OnDraw(MDrawContext* pDC)
{
	...
	if ( ZGetMyInfo()->IsAdminGrade())
	{
		...
		char szName[128];
		sprintf( szName, "%s (HP:%d, AP:%d)", m_pTargetCharacter->GetUserName(), m_pTargetCharacter->GetHP(), m_pTargetCharacter->GetAP());
