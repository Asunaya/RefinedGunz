...
//			if ( bSelected && !bFulled)
			...
			if ( (*itr)->bIsLive)
			{
				if ( (*itr)->nNumOfUser >= (*itr)->nCapacity)
					sprintf( szText, "%s (FULL / %d)", (*itr)->szName, (*itr)->nCapacity);
				else
					sprintf( szText, "%s (%d / %d)", (*itr)->szName, (*itr)->nNumOfUser, (*itr)->nCapacity);
			}
			else
			{
				sprintf( szText, "%s (checking)", (*itr)->szName);
