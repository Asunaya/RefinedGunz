...
#define NEXTLINE() { sy+=15; }
	...
	if(m_nShowDebugInfo==2) {
		...
		sprintf(buffer, "observer cmd q=%d , effect 0=%d ,1=%d, 2=%d   seffect = %d, dynlight = %d",g_pGame->GetObserverCommandListCount(),
	...
	{
	case MMATCH_ROUNDSTATE_PREPARE: sprintf(buffer, "RoundState: Wait"); break;
	case MMATCH_ROUNDSTATE_COUNTDOWN: sprintf(buffer, "RoundState: CountDown"); break;
	case MMATCH_ROUNDSTATE_PLAY: sprintf(buffer, "RoundState: Play"); break;
	case MMATCH_ROUNDSTATE_FINISH: sprintf(buffer, "RoundState: Finish"); break;
	case MMATCH_ROUNDSTATE_EXIT: sprintf(buffer, "RoundState: Exit"); break;
	case MMATCH_ROUNDSTATE_FREE: sprintf(buffer, "RoundState: Free"); break;
	}
	OUTTEXT();
	NEXTLINE();
	...
	*/
	...
	if(g_pGame->m_pMyCharacter->m_bGuard)
	{
		...
		{
			char buf[256];
			sprintf(buf,"bl%d",g_pGame->m_pMyCharacter->m_nGuardBlock);
			strcat(buffer,buf);
		}
...
//		ZGetGame()->GetWorld()->GetBsp()->DrawAdjPoltyongs();
		...
		*/
		...
		sprintf(buffer,"dist to floor = %6.3f ", g_pGame->m_pMyCharacter->GetDistToFloor());
		pDC->SetColor(MCOLOR(0xFFffffff));
		OUTTEXT();
		NEXTLINE();
...
void ZScreenDebugger::DrawActorInfo(int num, MDrawContext *pDC, ZActor* pActor)
{
	...
	sprintf(buf, "     FLAG: LAND(%s), BLAST(%s), BLASTDAGGER(%s), MOVING(%s), DEAD(%s), REQUESTED_DEAD(%s), MY_CONTROL(%s)",
