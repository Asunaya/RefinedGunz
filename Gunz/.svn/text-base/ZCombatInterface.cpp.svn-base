...
//		if(g_pGame&&g_pGame->GetMatch()->GetRoundState()==MMATCH_ROUNDSTATE_PLAY)
		{
			...
			sprintf(buffer,"%d  %s", pCharacter->GetProperty()->nLevel, pCharacter->GetProperty()->szName);
			...
			{
				sprintf(buffer,"%d / %d", m_nBulletAMagazine, m_nBullet);
	...
	if (pMatch->GetRoundState() == MMATCH_ROUNDSTATE_PLAY)
	{
		if (!pMatch->IsWaitForRoundEnd())
		{
			if (g_pGame->m_pMyCharacter && g_pGame->m_pMyCharacter->IsDie())
				...
				{
//					sprintf(szMsg, "%d 초동안 기다려주세요!", nRemainTime);
					char temp[ 4 ];
				...
				{
//					sprintf(szMsg, "플레이하려면 Fire키를 눌러주세요!");
					sprintf( szMsg, 
...
//	bool bClanGame = true;
	...
	if (ZApplication::GetGame()->GetMatch()->IsTeamPlay())
	{
		...
		else
		{
			if (ZApplication::GetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM2) // 팀전일때 무한데스매치만 예외가 많이 발생합니다 =_=;
				sprintf(szText, "%s : %d(Red) vs %d(Blue)",  ZGetGameTypeManager()->GetGameTypeStr(g_pGame->GetMatch()->GetMatchType()),
		...
		sprintf(szText, "%s", ZGetGameTypeManager()->GetGameTypeStr(g_pGame->GetMatch()->GetMatchType()));
	...
	{
		sprintf( szText, "%s : %d", ZMsg( MSG_WORD_REMAINNPC), ZGetQuest()->GetGameInfo()->GetNPCCount() - ZGetQuest()->GetGameInfo()->GetNPCKilled());
		TextRelative( pDC, x, y, szText);
		y -= linespace2;
	}
	...
	if ( ZApplication::GetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DUEL)
	{
		...
		sprintf( szText, "%s : %d", ZMsg( MSG_WORD_ENDKILL), g_pGame->GetMatch()->GetRoundCount());
	}
	...
	else if ( ZApplication::GetGame()->GetMatch()->IsWaitForRoundEnd() && !bClanGame)
	{
		...
		sprintf( szText, "%s : %d / %d %s", ZMsg( MSG_WORD_RPROGRESS), g_pGame->GetMatch()->GetCurrRound() + 1, g_pGame->GetMatch()->GetRoundCount(), ZMsg( MSG_WORD_ROUND));
	}
...
//	const float clanXPOS[6] = {0.45f, 0.25f, 0.68f, 0.77f, 0.83f, 0.92f};
	...
	sprintf( szBuff, "%s   %s", ZMsg(MSG_CHARINFO_LEVEL), ZMsg(MSG_CHARINFO_NAME));
...
//	int nMaxLineCount=int((fHeight-fTitleHeight)*(float)RGetScreenHeight()/((float)pFont->GetHeight()*1.1f));
	...
	if(bClanGame)
	{
		for(int i=0;i<2;i++)
		{
			...
			// 점수 출력. 가운데 정렬
			sprintf(szText,"%d",g_pGame->GetMatch()->GetTeamScore(nTeam));
...
//		backgroundy=newbackgroundy;
		...
		if ( ZGetGameTypeManager()->IsQuestDerived( g_pGame->GetMatch()->GetMatchType()))
		{
			...
			if ( itor != ZApplication::GetGame()->m_CharacterManager.end())
			{
				...
				x=ITEM_XPOS[2];
//				sprintf(szText,"%d/%d", pQuestPlayerInfo->GetHP(), pQuestPlayerInfo->GetAP());
	...
	if(bClanGame)
	{
		...
		for(ZResultBoardList::iterator i=m_ResultItems.begin();i!=m_ResultItems.end();i++)
		{
			...
			if(bDrawClanName)
			{
				...
				char szText[32];
				sprintf(szText,"%d",g_pGame->GetMatch()->GetTeamScore((MMatchTeam)pItem->nTeam));
...
//	m_pResultPanel=ZGetScreenEffectManager()->CreateScreenEffect("ef_in_result.elu");
		...
		itor != ZGetCharacterManager()->end(); ++itor)
	{
		...
		strcpy(pItem->szClan,pCharacter->GetProperty()->szClanName);
		pItem->nClanID = pCharacter->GetClanID();
		pItem->nTeam = ZApplication::GetGame()->GetMatch()->IsTeamPlay() ? pCharacter->GetTeamID() : MMT_END;
	...
	if(bClanGame) {
		if (pChar->GetTeamID() == MMT_RED) {
			if(m_nClanIDRed==0) {
				...
				strcpy(m_szRedClanName,pChar->GetProperty()->szClanName);
			}
		}
			...
			if(m_nClanIDBlue==0) {
				...
				strcpy(m_szBlueClanName,pChar->GetProperty()->szClanName);
			}
		}
	}
}
