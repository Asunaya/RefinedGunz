...
*/
	...
	if ( pPicture)
	{
		sprintf( szMapName, "interface/loadable/%s", MGetMapImageName( ZGetGameClient()->GetMatchStageSetting()->GetMapName()));

		// 임시 하드코딩 우에엥~~~
		if ( m_nGameType == MMATCH_GAMETYPE_QUEST)
			strcpy( szMapName, "interface/loadable/map_Mansion.bmp");
...
//	}
	...
	for ( int i = 0;  i < rootElement.GetChildNodeCount();  i++)
	{
		...
		if ( !stricmp( szTagName, "SPECIAL_SCENARIO"))			// 태그 시작
		{
			char szAttrName[64];
