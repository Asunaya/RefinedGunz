...
public:
	...
	void SetAuthCookie(const char* pszVal)	{ strcpy_safe(m_AuthCookie, pszVal);	}
	const char* GetDataCookie()				{ return m_DataCookie;			}
	void SetDataCookie(const char* pszVal)	{ strcpy_safe(m_DataCookie, pszVal);	}
	const char* GetSpareParam()				{ return m_SpareParam;			}
	void SetSpareParam(const char* pszVal)	{ strcpy_safe(m_SpareParam, pszVal);	}
};
